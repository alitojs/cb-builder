  ...
  <div class="actualPersonStyle">
    ...
    <a-card class="oldman-bottom">
      ...
      <div class="table-operator">
        <div class="top-left">
          <card-title title="执勤任务列表" background="white" :fontWeight="true" fontSize="20px" />
        </div>
        <div class="topl-commandNumber">
          <a-button type="primary" icon="swap" @click="batchingClick">批量处理</a-button>
          <bath-download-progress
            :downloadTotal="ipagination.total"
            :customUrl="url.exportXlsUrl"
            fileName="人口信息"
            :getParams="handleExportXlsParams">
            <a-button type="primary" icon="download" class="orgClass">导出</a-button>
          </bath-download-progress>
        </div>
      </div>
...
import BathDownloadProgress from '@/components/publicSecurity/Download/BatchDownloadProgress';

export default {
  name: 'actualPerson',
  mixins: [AutoJeecgListMixin, mixinDevice, myPublicMixins],
  components: { BathDownloadProgress },
  data() {
    return {
      ...
      selectionFlag: false, // 打开批量操作的标识
      selectedRows: [], // 所选的数据
      MENUS: {},
      showMore: false, // 是否展示更多的标识
      closeCol: {
        xl: 6,
        lg: 8,
        md: 12,
